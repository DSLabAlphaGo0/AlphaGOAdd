// 使用模块化的形式，将多个js文件放到这一个文件中，相互调用。
// 所以可以不看这个，单独看JGO文件夹下的单独的文件，可读性更高。
 ...
 * Clear probalities
 */
Board.prototype.clearP = function() {
  this.each(function(c) {
    this.setPro(c, C.CLEAR);
    this.setMark(c, C.MARK.NONE);
  }.bind(this));
};

Board.prototype.setPro = function(c, t) {
  if(c instanceof Coordinate) {
    var old = this.probalities[c.i][c.j];

    if(old == t) return; // no change

    this.probalities[c.i][c.j] = t;
...
Canvas.prototype.draw = function(jboard, i1, j1, i2, j2) {
  ...
  jboard.eachP(function(c, probalities, mark) {
    if(this.probalities != 0) {
      var ox = (this.getX(c.i - this.opt.view.xOffset));
      var oy = (this.getY(c.j - this.opt.view.yOffset));
      var markColor;

      this.ctx.globalAlpha = this.opt.stone.dimAlpha * probalities*50;
